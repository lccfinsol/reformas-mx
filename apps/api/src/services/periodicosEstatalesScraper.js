  ...
  for (const [estado, url] of Object.entries(ESTADOS_PERIODICOS)) {
    try {
      ...
      const reformas = publicaciones.map(publicacion => ({
        ...
        contenido: publicacion.titulo,
        fecha_publicacion: fechaFormato,
        fuente: 'Periódicos Oficiales de Estados',
        nivel: 'Estatal',
        url_fuente: publicacion.url,
        materia_legal: clasificarMateria(publicacion.titulo),
        estado,
      }));
...
export { scrapePeriodicosEstatales, obtenerPaginaEstatal, extraerPublicaciones, clasificarMateria };
