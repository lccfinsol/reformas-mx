  ...
  try {
    ...
    for (const reforma of reformas) {
      try {
        ...
        if (!isValidUrl(reforma.url_fuente)) {
          errors.push(`Saltada reforma con URL inválida: ${reforma.url_fuente}`);
          continue;
        }
        ...
        if (isDuplicate) {
          logger.debug(`Reforma duplicada saltada: ${reforma.url_fuente}`);
          continue;
        }
